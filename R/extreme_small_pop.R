# script to run simulations for wolbachia infection frequency model without CI
...
N_val_vals <- c(100) # population size
F_m_vals <- c(1.025) # fecundity 
F_cv_vals <- c(0.1) # coefficient of variation for F
# transmission rates and binomial proportions
mu_vect_vals <- list(c(0.01,0.8))
...
saveRDS(sim_no_ci_df[seq(1,nrow(sim_no_ci_df),by=2), ],
        file = "mu_sims_data/data_for_abs_change_extreme_small_pop.rds")
